const ServicesBlock = () => {
  return (
    <section id="services" className="p-5 text-center mx-auto bg-gray-900">
      <h2 className="text-4xl sm:text-6xl uppercase font-bold tracking-wider py-2 my-5 text-gray-50">
        Our Services
      </h2>

      <div className="sm:grid sm:grid-cols-2 lg:grid-cols-3">
        <div className="bg-gray-900 rounded-md strong-shadow m-4 p-2 flex flex-col items-center">
          <div className="service w-full p-5 m-2 bg-center bg-scroll bg-cover flex place-content-center place-items-center">
            <h2 className="font-light italic text-2xl sm:text-4xl text-center tracking-wider text-gray-50">
              Expert Horticulture
            </h2>
          </div>
          <p className="sm:text-xl text-gray-50 font-light py-2 my-2">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed sequi
            quis et dolore quia incidunt excepturi id a earum labore!
          </p>

          <p className="text-2xl font-light py-2 my-2">
            <a className="text-gray-300" href="#">
              Learn more
            </a>
          </p>
        </div>

        <div className="bg-gray-900 rounded-md strong-shadow m-4 p-2 flex flex-col items-center">
          <div className="service w-full p-5 my-2 bg-center bg-scroll bg-cover flex place-content-center place-items-center">
            <h2 className="font-light italic text-2xl sm:text-4xl text-center tracking-wider text-gray-50">
              Water Features
            </h2>
          </div>
          <p className="sm:text-xl text-gray-50 font-light py-2 my-2">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed sequi
            quis et dolore quia incidunt excepturi id a earum labore!
          </p>

          <p className="text-2xl font-light py-2 my-2">
            <a className="text-gray-300" href="#">
              Learn more
            </a>
          </p>
        </div>

        <div className="bg-gray-900 rounded-md strong-shadow m-4 p-2 flex flex-col items-center">
          <div className="service w-full p-5 my-2 bg-center bg-scroll bg-cover flex place-content-center place-items-center">
            <h2 className="font-light italic text-2xl sm:text-4xl text-center tracking-wider text-gray-50">
              Exterior Carpentry
            </h2>
          </div>
          <p className="sm:text-xl text-gray-50 font-light py-2 my-2">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed sequi
            quis et dolore quia incidunt excepturi id a earum labore!
          </p>

          <p className="text-2xl font-light py-2 my-2">
            <a className="text-gray-300" href="#">
              Learn more
            </a>
          </p>
        </div>

        <div className="bg-gray-900 rounded-md strong-shadow m-4 p-2 flex flex-col items-center">
          <div className="service w-full p-5 my-2 bg-center bg-scroll bg-cover flex place-content-center place-items-center">
            <h2 className="font-light italic text-2xl sm:text-4xl text-center tracking-wider text-gray-50">
              Hardscapes
            </h2>
          </div>
          <p className="sm:text-xl text-gray-50 font-light py-2 my-2">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed sequi
            quis et dolore quia incidunt excepturi id a earum labore!
          </p>

          <p className="text-2xl font-light py-2 my-2">
            <a className="text-gray-300" href="#">
              Learn more
            </a>
          </p>
        </div>

        <div className="bg-gray-900 rounded-md strong-shadow m-4 p-2 flex flex-col items-center">
          <div className="service w-full p-5 my-2 bg-center bg-scroll bg-cover flex place-content-center place-items-center">
            <h2 className="font-light italic text-2xl sm:text-4xl text-center tracking-wider text-gray-50">
              Landscape Design
            </h2>
          </div>
          <p className="sm:text-xl text-gray-50 font-light py-2 my-2">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed sequi
            quis et dolore quia incidunt excepturi id a earum labore!
          </p>

          <p className="text-2xl font-light py-2 my-2">
            <a className="text-gray-300" href="#">
              Learn more
            </a>
          </p>
        </div>

        <div className="bg-gray-900 rounded-md strong-shadow m-4 p-2 flex flex-col items-center">
          <div className="service w-full p-5 my-2 bg-center bg-scroll bg-cover flex place-content-center place-items-center">
            <h2 className="font-light italic text-2xl sm:text-4xl text-center tracking-wider text-gray-50">
              Maintenance
            </h2>
          </div>
          <p className="sm:text-xl text-gray-50 font-light py-2 my-2">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Sed sequi
            quis et dolore quia incidunt excepturi id a earum labore!
          </p>

          <p className="text-2xl font-light py-2 my-2">
            <a className="text-gray-300" href="#">
              Learn more
            </a>
          </p>
        </div>
      </div>
    </section>
  );
};

export default ServicesBlock;
